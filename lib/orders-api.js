...
const ITEM_STATES = new Set(["pendiente", "armado", "faltante"]);

let operationalSchemaCache = { checkedAt: 0, available: false };
let missingSchemaCache = { checkedAt: 0, available: false };
...
async function supabaseRequest(path, options = {}) {
  ...
  if (!response.ok) {
    throw new Error(`Supabase ${response.status}: ${text.slice(0, 1400)}`);
  }
...
async function missingSchemaAvailable(force = false) {
  const now = Date.now();
  if (!force && now - missingSchemaCache.checkedAt < SCHEMA_CACHE_MS) {
    return missingSchemaCache.available;
  }

  try {
    await supabaseRequest(
      "pedido_items?select=faltante,faltante_resuelto,faltante_marcado_en,faltante_resuelto_en&limit=1",
      { method: "GET", prefer: "return=representation" },
    );
    missingSchemaCache = { checkedAt: now, available: true };
  } catch (error) {
    console.warn("[orders] Migración de faltantes todavía no disponible:", String(error));
    missingSchemaCache = { checkedAt: now, available: false };
  }
...
async function requireMissingSchema() {
  if (await missingSchemaAvailable(true)) return;
  const error = new Error("Falta ejecutar la migración de artículos faltantes en Supabase");
  error.statusCode = 503;
  error.code = "MISSING_ITEMS_SCHEMA_NOT_CONFIGURED";
  throw error;
}
...
async function mergeItems(orderId, items) {
  if (!items.length) return [];
  const missingEnabled = await missingSchemaAvailable();
  const select = missingEnabled
    ? "producto_id,cantidad,subtotal_uyu,consultable,sector,faltante,faltante_resuelto"
    : "producto_id,cantidad,subtotal_uyu,consultable,sector";
  const existingRows = await supabaseRequest(
    `pedido_items?select=${select}&pedido_order_id=eq.${encodeURIComponent(orderId)}`,
    { method: "GET", prefer: "return=representation" },
  );
  ...
  const records = items.map((item) => {
    ...
    if (missingEnabled) {
      record.faltante = false;
      record.faltante_resuelto = false;
      record.faltante_marcado_en = null;
      record.faltante_resuelto_en = null;
    }
    return record;
  });
...
async function loadOrderItems(orderId, missingEnabled = false) {
  const select = missingEnabled
    ? "pedido_order_id,producto_id,producto_nombre,cantidad,precio_unitario_uyu,subtotal_uyu,consultable,armado,sector,actualizado_en,faltante,faltante_resuelto,faltante_marcado_en,faltante_resuelto_en"
    : "pedido_order_id,producto_id,producto_nombre,cantidad,precio_unitario_uyu,subtotal_uyu,consultable,armado,sector,actualizado_en";
  const rows = await supabaseRequest(
    `pedido_items?select=${select}&pedido_order_id=eq.${encodeURIComponent(orderId)}&order=sector.asc,producto_nombre.asc`,
    { method: "GET", prefer: "return=representation" },
  );
  return Array.isArray(rows) ? rows : [];
}

function assemblyProgress(items) {
  const total = items.length;
  const prepared = items.filter((item) => Boolean(item.armado)).length;
  const missing = items.filter((item) => Boolean(item.faltante)).length;
  const handled = Math.min(total, prepared + missing);
...
function automaticAssemblyStatus(progress) {
  if (progress.total > 0 && progress.pending === 0) return "armado";
  if (progress.handled > 0) return "armando";
  return "pendiente";
}
...
async function saveNewOperationalOrder(order, client) {
  ...
  return { ...saved, merged: false, duplicate: false, incomingOrderId: order.orderId };
}

async function mergeIntoOperationalOrder(active, order, client) {
  await mergeItems(active.order_id, order.items);
  const missingEnabled = await missingSchemaAvailable();
  const progress = assemblyProgress(await loadOrderItems(active.order_id, missingEnabled));
  const nextArmStatus = automaticAssemblyStatus(progress);
  const now = new Date().toISOString();
  ...
  return { ...saved, merged: true, duplicate: false, incomingOrderId: order.orderId };
}
...
async function handleOrderPost(req, res) {
  ...
  if (!order) return sendJson(res, 400, { ok: false, error: "INVALID_ORDER" });
...
async function loadActiveOrdersForPanel(missingEnabled) {
  ...
  if (safeIds.length) {
    const select = missingEnabled
      ...
      : "pedido_order_id,producto_id,producto_nombre,cantidad,precio_unitario_uyu,subtotal_uyu,consultable,armado,sector,actualizado_en";
    const rows = await supabaseRequest(
      `pedido_items?select=${select}&pedido_order_id=in.(${safeIds.join(",")})&order=sector.asc,producto_nombre.asc`,
      { method: "GET", prefer: "return=representation" },
    );
  ...
  for (const item of items) {
    ...
    byOrder.get(key).push({
      ...item,
      faltante: missingEnabled ? Boolean(item.faltante) : false,
      faltante_resuelto: missingEnabled ? Boolean(item.faltante_resuelto) : false,
    });
  }

  return list.map((order) => {
    const orderItems = byOrder.get(toStr(order.order_id)) || [];
    const progress = assemblyProgress(orderItems);
...
async function loadMissingItemsSummary() {
  const rows = await supabaseRequest(
    "pedido_items?select=pedido_order_id,producto_id,producto_nombre,cantidad,sector,faltante_marcado_en&faltante=eq.true&faltante_resuelto=eq.false&order=faltante_marcado_en.asc",
    { method: "GET", prefer: "return=representation" },
  );
  const grouped = new Map();

  for (const row of Array.isArray(rows) ? rows : []) {
    const productId = toStr(row.producto_id);
    if (!productId) continue;
    const markedAt = toStr(row.faltante_marcado_en) || null;
    const current = grouped.get(productId) || {
      producto_id: productId,
      producto_nombre: toStr(row.producto_nombre) || "Producto sin nombre",
      sector: toStr(row.sector) || "Sin sector",
      cantidad_total: 0,
      pedidos: [],
      marcado_primero_en: markedAt,
      marcado_ultimo_en: markedAt,
    };

    current.cantidad_total += clampInteger(row.cantidad, 0, 1000000);
    if (row.pedido_order_id && !current.pedidos.includes(row.pedido_order_id)) {
      current.pedidos.push(row.pedido_order_id);
    }
    if (markedAt) {
      if (!current.marcado_primero_en || markedAt < current.marcado_primero_en) {
        current.marcado_primero_en = markedAt;
      }
      if (!current.marcado_ultimo_en || markedAt > current.marcado_ultimo_en) {
        current.marcado_ultimo_en = markedAt;
      }
    }
...
async function updateOrderAssemblyFromItems(orderId, missingEnabled) {
  const items = await loadOrderItems(orderId, missingEnabled);
  const progress = assemblyProgress(items);
  const status = automaticAssemblyStatus(progress);
  const rows = await supabaseRequest(
    `pedidos?order_id=eq.${encodeURIComponent(orderId)}`,
    {
      ...
      body: JSON.stringify({
        estado: status,
        estado_armado: status,
        actualizado_en: new Date().toISOString(),
      }),
    },
  );
  return { order: Array.isArray(rows) ? rows[0] || null : null, progress };
}

async function setItemStatus(orderId, productId, status) {
  if (!ITEM_STATES.has(status)) {
    const error = new Error("Estado de artículo inválido");
  ...
  const missingEnabled = await missingSchemaAvailable();
  if (status === "faltante" && !missingEnabled) await requireMissingSchema();
  const now = new Date().toISOString();
  const patch = {
    armado: status === "armado",
    actualizado_en: now,
  };

  if (missingEnabled) {
    patch.faltante = status === "faltante";
    patch.faltante_resuelto = false;
    patch.faltante_marcado_en = status === "faltante" ? now : null;
    patch.faltante_resuelto_en = null;
  }
  ...
  return updateOrderAssemblyFromItems(orderId, missingEnabled);
}

async function setOrderAssemblyStatus(orderId, requestedStatus) {
  if (!(await loadActivePanelOrder(orderId))) {
    const error = new Error("El pedido ya no está activo en el panel");
    error.statusCode = 404;
    error.code = "ORDER_NOT_ACTIVE";
  ...
  const missingEnabled = await missingSchemaAvailable();
  const progress = assemblyProgress(await loadOrderItems(orderId, missingEnabled));
  let status;
  if (requestedStatus === "armado") status = "armado";
  else if (requestedStatus === "reabrir") status = progress.handled > 0 ? "armando" : "pendiente";
  else {
    const error = new Error("Estado de armado inválido");
    error.statusCode = 400;
    error.code = "INVALID_ASSEMBLY_STATUS";
    throw error;
  }
...
async function resolveMissingProduct(productId) {
  await requireMissingSchema();
  const now = new Date().toISOString();
  const rows = await supabaseRequest(
    `pedido_items?producto_id=eq.${encodeURIComponent(productId)}&faltante=eq.true&faltante_resuelto=eq.false`,
    {
      method: "PATCH",
      prefer: "return=representation",
      body: JSON.stringify({
        faltante_resuelto: true,
        faltante_resuelto_en: now,
        actualizado_en: now,
      }),
    },
  );
  if (!Array.isArray(rows) || !rows.length) {
    const error = new Error("El artículo ya no figura como faltante");
    error.statusCode = 404;
    error.code = "MISSING_ITEM_NOT_FOUND";
    throw error;
  }
  return { resolved_rows: rows.length, producto_id: productId };
}
...
async function handlePanelGet(req, res) {
  ...
  const missingEnabled = await missingSchemaAvailable();
  const [orders, missingItems] = await Promise.all([
    loadActiveOrdersForPanel(missingEnabled),
    missingEnabled ? loadMissingItemsSummary() : Promise.resolve([]),
  ]);
  ...
  return sendJson(res, 200, {
    ...
    missing_features_enabled: missingEnabled,
    orders,
...
async function handlePanelPatch(req, res) {
  ...
  if (action === "resolve_missing_product") {
    ...
    if (!productId) return sendJson(res, 400, { ok: false, error: "INVALID_PRODUCT_ID" });
    const result = await resolveMissingProduct(productId);
  ...
  if (action === "set_item_status") {
    const productId = toStr(body.productId).slice(0, 160);
    if (!productId) return sendJson(res, 400, { ok: false, error: "INVALID_PRODUCT_ID" });
    const result = await setItemStatus(orderId, productId, toStr(body.status));
    return sendJson(res, 200, { ok: true, ...result });
  }
  ...
  if (action === "toggle_item") {
    const productId = toStr(body.productId).slice(0, 160);
    if (!productId) return sendJson(res, 400, { ok: false, error: "INVALID_PRODUCT_ID" });
    const result = await setItemStatus(orderId, productId, body.armed ? "armado" : "pendiente");
    return sendJson(res, 200, { ok: true, ...result });
  }

  if (action === "set_order_assembly_status") {
    const result = await setOrderAssemblyStatus(orderId, toStr(body.status));
    return sendJson(res, 200, { ok: true, ...result });
  }
