...
async function backfillCompletedAssignedOrders() {
  const { response, data } = await request(
    "pedidos?select=order_id,estado_armado,asignado_usuario_id,asignado_usuario,asignado_nombre&estado_armado=eq.armado&asignado_usuario_id=not.is.null",
    { method: "GET" },
  );
  if (!response.ok || !Array.isArray(data) || !data.length) return;

  await Promise.allSettled(
    data.map((order) => recordStaffPerformance(order.order_id, {
      id: toStr(order.asignado_usuario_id),
      username: toStr(order.asignado_usuario),
      name: toStr(order.asignado_nombre) || toStr(order.asignado_usuario) || "Empleado",
      role: "empleado",
      active: true,
    })),
  );
...
export async function listStaffPerformance({ period = "30", from = "", to = "" } = {}) {
  await backfillCompletedAssignedOrders();
